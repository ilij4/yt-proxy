...
import { VideoLinkDocument, VideoLinkModel } from "../models/video-link";
import {
...
async function getVideoByHash(hash: string) {
  const video = await VideoLinkModel.findOne({ hash });
  if (!video) {
...
async function touchVideoAccess(videoId: string): Promise<void> {
  await VideoLinkModel.updateOne(
    { videoId },
...
export async function registerVideoRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: CreateVideoBody }>("/api/videos", {
    schema: {
      tags: ["Videos"],
    ...
    const [existingByHash, existingByUrl, existingByVideoId] = await Promise.all([
      VideoLinkModel.findOne({ hash }),
      VideoLinkModel.findOne({ url: normalizedUrl }),
      VideoLinkModel.findOne({ videoId })
    ]);
    ...
    if (existingByHash || existingByUrl || existingByVideoId) {
      ...
      if (record && payload.userId && !record.userId) {
    ...
    const created = await VideoLinkModel.create({
      url: normalizedUrl,
      hash,
      videoId,
  ...
  app.get("/api/videos", {
    ...
    const filter: FilterQuery<VideoLinkDocument> = {};
    ...
    if (tags.length === 0) {
      const [list, count] = await Promise.all([
        VideoLinkModel.find(filter).sort({ [sortField]: sortOrder }).skip(skip).limit(query.limit),
        VideoLinkModel.countDocuments(filter)
      ]);
      items = list;
      total = count;
    } else {
      const metadataCollectionName = VideoMetadataCacheModel.collection.name;
      const tagsMatch =
        query.tagsMode === "all"
  ...
  app.get<{ Params: HashParams }>("/api/videos/:hash", {
    schema: {
      tags: ["Videos"],
  ...
  app.patch<{ Params: HashParams }>("/api/videos/:hash", {
    schema: {
      tags: ["Videos"],
    ...
    const update: Partial<Pick<VideoLinkDocument, "userId" | "elo" | "category">> = {};
    ...
    const video = await VideoLinkModel.findOneAndUpdate(
  ...
  app.get<{ Params: HashParams; Querystring: RefreshQuerystring }>("/api/videos/:hash/metadata", {
    schema: {
      tags: ["Videos"],
  ...
  app.get<{ Params: HashParams; Querystring: RefreshQuerystring }>("/api/videos/:hash/thumbnail", {
    schema: {
      tags: ["Videos"],
  ...
  app.get<{ Params: HashParams; Querystring: RefreshQuerystring }>("/api/videos/:hash/stats", {
    schema: {
      tags: ["Videos"],
  ...
  app.get<{ Params: HashParams; Querystring: RefreshQuerystring }>("/api/videos/:hash/channel", {
    schema: {
      tags: ["Videos"],
