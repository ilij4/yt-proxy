...
export async function fetchVideoParts(videoId: string, parts: VideoPart[]): Promise<YouTubeApiVideoPartsResult> {
    if (parts.length === 0) {
        return {};
    }
...
export async function fetchVideoMetadata(videoId: string): Promise<YouTubeApiVideoMetadata> {
    const parts = await fetchVideoParts(videoId, [
